  ...
  return (
    <section className="bg-white dark:bg-[#191919]">
      {features.map((feature, index) => (
        <div className="max-w-screen-2xl mx-auto gap-16 xl:gap-[10rem] lg:grid lg:grid-cols-2 px-4 py-8 lg:py-14">
          <div
            ...
            <ul role="list" className="mb-8 space-y-4 text-left mt-12 pt-12">
              {feature.features.map((feature) => (
                <li className="flex space-x-3">
                  <div className="mt-1">
                    <HiOutlineCheck className="text-primary-600 dark:text-green-400" />
                  </div>
                  <div>
          ...
          <img className="rounded-md" draggable={false} src={feature.image} alt={feature.title} />
        </div>
      ))}
    </section>
  );
